...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] = for ( (block,moves) <- neighbors if !explored.contains(block) ) yield (block,moves)
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter( p => p match { case (b, _) => b.b1 == goal && b.b2 == goal } )
